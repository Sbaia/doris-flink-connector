...
public final class ArrowSerializer {
    ...
    /** Channel wrapper for compressed writes (requires WritableByteChannel, not OutputStream). */
    private transient WritableByteChannel channel;

    public ArrowSerializer(RowType inputType, RowType outputType) {
        this(inputType, outputType, false);
    }

    public ArrowSerializer(RowType inputType, RowType outputType, boolean enableZstdCompression) {
    ...
    public void open(InputStream bais, OutputStream baos) throws Exception {
        ...
        arrowStreamWriter = createStreamWriter();
    ...
    /**
     * Creates an ArrowStreamWriter, optionally with ZSTD per-buffer compression.
     *
     * <p>The compression constructor requires a {@link WritableByteChannel} instead of an
     * OutputStream.
     */
    private ArrowStreamWriter createStreamWriter() {
