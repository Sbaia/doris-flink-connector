...
The manually dispatched `publish-poc-package.yml` workflow builds with JDK 21, runs the connector tests, publishes the reactor with the repository-scoped `GITHUB_TOKEN`, resolves it from an empty Maven cache, enforces dependency convergence, and compares the downloaded JAR SHA-256 with the locally built artifact. No personal access token is stored in this repository or in the pipeline repository.
